"""Macro Regime — the business-cycle classification that conditions everything."""
...
from macro_portfolio.research import regime as R   # noqa: E402
...
st.caption("Each month is labelled from composite PMI (above/below 50) and its "
           "3-month momentum, with CPI as context. This is the macro agent's job in "
           "Ang et al. (2026) — done as a transparent rules-based classifier.")
...
fig = go.Figure()
# colored bands for contiguous regime segments
seg_start = r.index[0]
...
fig.add_trace(go.Scatter(x=r.index, y=r["PMI"], mode="lines",
                         line=dict(color=theme.WHITE, width=2), name="Composite PMI"))
...
st.caption("Annualized mean return per asset, conditioned on the regime that month "
           "(in-sample). Equities lead in Recovery/Expansion and fall hardest in "
           "Contraction, where bonds hold up — the case for the macro tilt.")
